...
-type(amqp_method_field_name() ::
       ( nowait | ticket | ticket | cluster_id
       | locale | consumer_tag | response | requeue
       | exchange | message_count | meta_data | integer_3
       | consumer_tag | queue | reply_text | queue
       | exchange | delivery_tag | routing_key | nowait
       | reply_code | method_id | no_ack | string_1
       | write | consumer_tag | heartbeat | type
       | content_size | challenge | prefetch_size | delivery_tag
       | ticket | consumer_tag | frame_max | ticket
       | reply_code | consumer_tag | redelivered | consumer_tag
       | exchange | table | active | active
       | global | version_major | message_count | class_id
       | out_of_band | no_ack | mandatory | routing_key
       | arguments | exchange | message_count | ticket
       | requeue | prefetch_count | immediate | known_hosts
       | delivery_tag | no_local | method_id | queue
       | multiple | read | delivery_tag | prefetch_count
       | exchange | delivery_tag | reply_text | consumer_tag
       | nowait | passive | exchange | ticket
       | virtual_host | nowait | channel_max | arguments
       | integer_1 | immediate | routing_key | requeue
       | nowait | consumer_tag | class_id | ticket
       | delivery_tag | operation | ticket | exclusive
       | active | prefetch_size | string_result | identifier
       | reply_code | queue | if_empty | exclusive
       | consumer_tag | delivery_tag | version_minor | routing_key
       | auto_delete | queue | multiple | nowait
       | identifier | routing_key | insist | result
       | routing_key | reply_text | nowait | result
       | ticket | server_properties | mandatory | integer_result
       | nowait | ticket | if_unused | realm
       | global | ticket | exchange | consumer_tag
       | no_local | host | mechanisms | response
       | internal | consumer_tag | redelivered | integer_2
       | frame_max | nowait | immediate | exchange
       | capabilities | exclusive | exchange | delivery_tag
       | reply_code | locales | routing_key | exclusive
       | queue | details | consumer_tag | routing_key
       | queue | string_2 | consume_rate | redelivered
       | dtx_identifier | consumer_tag | if_unused | heartbeat
       | staged_size | mandatory | ticket | arguments
       | no_ack | durable | routing_key | reply_text
       | operation | consumer_tag | consumer_count | identifier
       | queue | passive | arguments | requeue
       | string_op | ticket | reply_text | content_checksum
       | ticket | nowait | nowait | prefetch_count
       | nowait | ticket | queue | consumer_tag
       | channel_max | client_properties | durable | integer_op
       | global | exchange | routing_key | mechanism
       | no_local | passive | exchange | reply_code
       | known_hosts | message_count | routing_key | exchange
       | queue | auto_delete | exchange | reply_text
       | queue | reply_code | integer_4 | exchange
       | exclusive | prefetch_size )).
