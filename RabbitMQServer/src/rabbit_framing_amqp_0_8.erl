...
-type(amqp_method_field_name() ::
       ( redelivered | consumer_tag | no_local | ticket
       | heartbeat | active | reply_text | exchange
       | nowait | integer_2 | immediate | exchange
       | cluster_id | ticket | prefetch_count | requeue
       | nowait | channel_max | consumer_tag | immediate
       | known_hosts | arguments | delivery_tag | method_id
       | queue | multiple | active | known_hosts
       | table | reply_code | reply_text | redelivered
       | exclusive | exchange | passive | message_count
       | nowait | nowait | version_minor | ticket
       | nowait | operation | message_count | arguments
       | delivery_tag | string_op | queue | mandatory
       | integer_op | ticket | nowait | consumer_tag
       | identifier | routing_key | nowait | delivery_tag
       | mechanism | reply_code | meta_data | operation
       | reply_code | delivery_tag | no_ack | exclusive
       | no_ack | consumer_tag | version_major | arguments
       | exclusive | content_size | ticket | delivery_tag
       | server_properties | prefetch_size | consumer_tag | ticket
       | requeue | insist | result | routing_key
       | reply_text | consumer_tag | multiple | active
       | class_id | integer_result | global | reply_code
       | message_count | consumer_tag | no_local | realm
       | queue | integer_3 | result | routing_key
       | message_count | identifier | response | internal
       | ticket | consumer_tag | frame_max | nowait
       | immediate | exchange | capabilities | exclusive
       | exchange | ticket | class_id | prefetch_count
       | out_of_band | consumer_tag | ticket | routing_key
       | ticket | exchange | routing_key | client_properties
       | auto_delete | exchange | channel_max | if_unused
       | mandatory | ticket | virtual_host | durable
       | nowait | reply_text | global | consumer_count
       | identifier | queue | details | arguments
       | mandatory | passive | reply_text | content_checksum
       | queue | integer_1 | if_empty | prefetch_count
       | consumer_tag | queue | exchange | consume_rate
       | prefetch_size | durable | delivery_tag | if_unused
       | consumer_tag | requeue | exchange | routing_key
       | requeue | routing_key | passive | exclusive
       | reply_code | nowait | exchange | mechanisms
       | routing_key | locale | nowait | routing_key
       | ticket | reply_text | prefetch_size | read
       | ticket | redelivered | consumer_tag | integer_4
       | consumer_tag | nowait | response | ticket
       | exchange | heartbeat | queue | no_ack
       | exchange | host | queue | consumer_tag
       | delivery_tag | routing_key | reply_code | auto_delete
       | string_2 | queue | exchange | string_result
       | exchange | method_id | queue | routing_key
       | write | delivery_tag | dtx_identifier | locales
       | type | queue | staged_size | challenge
       | global | consumer_tag | frame_max | ticket
       | no_local | string_1 )).
