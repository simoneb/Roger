...
-type(amqp_method_field_name() ::
       ( immediate | ticket | known_hosts | arguments
       | ticket | routing_key | routing_key | destination
       | insist | multiple | ticket | mechanism
       | channel_id | auto_delete | arguments | multiple
       | class_id | active | mandatory | exchange
       | version_major | durable | write | response
       | ticket | frame_max | exchange | ticket
       | capabilities | delivery_tag | method_id | requeue
       | locale | exclusive | nowait | delivery_tag
       | passive | consumer_count | nowait | passive
       | arguments | routing_key | routing_key | version_minor
       | exchange | if_empty | queue | redelivered
       | class_id | no_local | challenge | exclusive
       | reply_code | message_count | consumer_tag | exclusive
       | nowait | exchange | internal | nowait
       | exchange | arguments | nowait | nowait
       | client_properties | routing_key | ticket | delivery_tag
       | reply_text | queue | passive | queue
       | realm | routing_key | ticket | reply_text
       | virtual_host | ticket | queue | exchange
       | response | nowait | cluster_id | arguments
       | requeue | channel_max | no_ack | active
       | exchange | exchange | ticket | ticket
       | reply_code | server_properties | auto_delete | destination
       | redelivered | if_unused | nowait | delivery_tag
       | nowait | frame_max | nowait | global
       | delivery_tag | queue | ticket | heartbeat
       | consumer_tag | queue | reply_code | durable
       | if_unused | message_count | no_ack | routing_key
       | channel_max | routing_key | prefetch_count | consumer_tag
       | mechanisms | active | arguments | message_count
       | ticket | consumer_tag | read | type
       | consumer_tag | out_of_band | ticket | queue
       | reply_text | source | arguments | message_count
       | source | prefetch_size | queue | heartbeat
       | nowait | requeue | locales | method_id
       | requeue )).
