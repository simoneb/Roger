...
-type(amqp_method_field_name() ::
       ( ticket | consumer_tag | version_minor | reply_text
       | redelivered | message_count | channel_max | known_hosts
       | delivery_tag | auto_delete | ticket | routing_key
       | requeue | exchange | arguments | consumer_tag
       | ticket | realm | locale | queue
       | server_properties | internal | delivery_tag | nowait
       | exclusive | arguments | arguments | prefetch_size
       | arguments | exclusive | queue | nowait
       | active | nowait | consumer_tag | consumer_tag
       | cluster_id | nowait | routing_key | message_count
       | consumer_tag | queue | heartbeat | nowait
       | insist | nowait | active | exchange
       | ticket | auto_delete | multiple | message_count
       | immediate | response | arguments | version_major
       | durable | no_local | no_ack | mechanism
       | exchange | frame_max | routing_key | class_id
       | read | method_id | exclusive | active
       | delivery_tag | consumer_count | mandatory | if_unused
       | routing_key | if_empty | ticket | queue
       | source | virtual_host | source | ticket
       | requeue | durable | nowait | message_count
       | method_id | exchange | nowait | write
       | nowait | reply_code | heartbeat | ticket
       | nowait | challenge | client_properties | ticket
       | destination | routing_key | destination | reply_text
       | ticket | routing_key | passive | channel_id
       | queue | class_id | exchange | delivery_tag
       | type | capabilities | queue | exchange
       | response | queue | routing_key | routing_key
       | multiple | delivery_tag | ticket | reply_code
       | exchange | queue | no_ack | if_unused
       | reply_text | exchange | passive | requeue
       | locales | ticket | channel_max | mechanisms
       | arguments | nowait | frame_max | prefetch_count
       | arguments | ticket | redelivered | global
       | requeue | passive | ticket | out_of_band
       | reply_code )).
